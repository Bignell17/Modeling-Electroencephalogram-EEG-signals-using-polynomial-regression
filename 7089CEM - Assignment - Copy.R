setwd("C:/Users/Jordan/Desktop/7089 Assignment/7089CEM Assignment")
...
#Installing packages
install.packages("fitdistrplus")          #1.2
install.packages("logspline")             #1.2
install.packages("PerformanceAnalytics")  #1.3
...
bias <- matrix(1,201,1)
model1 <- cbind(bias, df$x4, df$x1^2, df$x1^3, df$x3^4)
model2 <- cbind(bias, df$x3^3, df$x3^4)
model3 <- cbind(bias, df$x2, df$x1^3, df$x3^4)
model4 <- cbind(bias, df$x4, (df$x1^3), (df$x3^4))
model5 <- cbind(bias, df$x4, df$x1^2, (df$x1^3), (df$x3^4), (df$x1^4))
...
p1 <- data.frame(test$time)


ggplot(test$time, yHatTest, aes(ymin=lowerCI, ymax=upperCI))+
...
maxabsoval <- matrix(cbind(abs(theta_hat3)))
mav1 <- maxabsoval[1,]
mav2 <- maxabsoval[2,]
mav3 <- cbind(mav1, mav2)
mav3  # parameters

sample_number = 3000

priorunibias <- runif(sample_number, (1.5) * (min = -mav1), (max = mav1)) # Computing 2 highest parameters for uniform distribution
prioruni1 <- runif(sample_number, (1.5) * (min = -mav2), (max = mav2))    

ABC_theta_hat = cbind(priorunibias, prioruni1, theta_hat3[3], theta_hat3[4]) # Create new theta hat


plot(priorunibias,prioruni1, main="ABC Rejection", xlab="Prior_Theta_bias", ylab="Prior_Theta1") # pre-plotting graph for for-loop

for (i in 1:sample_number) { # plotting all accepted theta based on epsilon 
  if ((1/201)* sum(df$y - model3 %*% ABC_theta_hat[i,])^2 < (0.8 * RSS_model3)) # Using epsilon in calculation with RSS as we have found this previously
  {points(ABC_theta_hat[i,1], ABC_theta_hat[i,2], col = "red")}
}

#theta tends to give the approx posterior distribution 

distance = (df$y - theta_hat3 * ABC_theta_hat)^2/201

data = data.frame(ABC_theta_hat)
library(LaplacesDemon)
joint.density.plot(accepted_thetas[,1], accepted_thetas[,2], Title=NULL, contour=TRUE, color=FALSE, Trace=NULL)

library(ggplot2)
windows(10,10)
geom_contour(
  mapping = NULL, data = data,stat = "contour", position = "identity", bins = NULL, binwidth = NULL, breaks = NULL,
  lineend = "butt", linejoin = "round", linemitre = 1, show.legend = NA,inherit.aes = TRUE
)
